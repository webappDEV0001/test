	...
	def update(self, request, *args, **kwargs):
		if 'number' in request.data:
			...
			if counter:
				...
				counter.save()
				return Response({"message": "Value has been updated", "value": Counter.objects.get(pk=1).number}, status=status.HTTP_200_OK)
			else:
				return Response({"message": "Value can not be reduced", "value": 1}, status=status.HTTP_400_BAD_REQUEST)
